...
1. **Apply the `Horse` → `Animal` rename migration to live Neon** (`prisma/migrations/20260718223633_rename_horse_to_animal`) — deliberately not run yet in this session pending James's explicit go-ahead, since it's a real schema change plus a data backfill (Binx → `species: CAT`) on production data. Once applied, the deployed app code also needs to move to the renamed routes/model at the same time (they're not backwards-compatible with each other) — this is a coordinated cutover, not two independent steps.
...
4. Once the rename is live in Neon, start `V2.md` Session 1 (Location model generalization) — the next stop was deliberately held until this rename landed and both suites confirmed green.
