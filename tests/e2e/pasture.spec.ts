...
test("an Admin moves a horse to a new field, closing out the previous assignment", async ({ adminPage }) => {
  ...
  await prisma.pastureAssignment.create({ data: { animalId: animal.id, fieldId: fieldL1.id, startDate: new Date("2026-06-01") } })
  ...
  const history = await prisma.pastureAssignment.findMany({ where: { animalId: animal.id, endDate: { not: null } } })
...
test("pasture assignment is Admin-only — a Shift Lead sees history read-only with no move form", async ({ shiftLeadPage }) => {
  ...
  await prisma.pastureAssignment.create({ data: { animalId: animal.id, fieldId: field.id, startDate: new Date("2026-06-01") } })
