...
import { createMedicationRegimen, endMedicationRegimen, logMedicationAdministered } from "@/app/horses/[id]/medication-actions"
...
describe("createMedicationRegimen", () => {
  it("is Admin-only — Shift Lead is rejected", async () => {
    ...
    await expect(
      createMedicationRegimen(horse.id, formData({ drugName: "Bute", dose: "1g", frequency: "Daily" }))
  ...
  it("creates the regimen with today as the start date for an Admin", async () => {
    ...
    const url = await captureRedirect(() =>
      createMedicationRegimen(horse.id, formData({ drugName: "Bute", dose: "1g", frequency: "Twice daily", route: "Oral, mixed in feed" }))
    )
...
describe("endMedicationRegimen", () => {
  it("is Admin-only — Shift Lead cannot end a regimen", async () => {
    ...
    const regimen = await prisma.medicationRegimen.create({
      data: { horseId: horse.id, drugName: "Bute", dose: "1g", frequency: "Daily", startDate: new Date("2026-01-01") }
    })
    await createVolunteer({ clerkId: "clerk_lead_er", role: "SHIFT_LEAD" })
    mockSignedInAs("clerk_lead_er")

    await expect(endMedicationRegimen(regimen.id, horse.id)).rejects.toThrow("Not authorized")
  ...
  it("sets endDate to today for an Admin", async () => {
    ...
    await captureRedirect(() => endMedicationRegimen(regimen.id, horse.id))
...
describe("logMedicationAdministered", () => {
  it("is Admin or Shift Lead — a plain Volunteer is rejected", async () => {
    ...
    await expect(logMedicationAdministered(regimen.id, horse.id, formData({ administered: "true" }))).rejects.toThrow("Not authorized")
  ...
  it("logs a given dose, attributed to the Shift Lead who logged it", async () => {
    ...
    await captureRedirect(() => logMedicationAdministered(regimen.id, horse.id, formData({ administered: "true", notes: "no issues" })))
  ...
  it("logs a missed dose", async () => {
    ...
    await captureRedirect(() => logMedicationAdministered(regimen.id, horse.id, formData({ administered: "false" })))
  ...
  it("is captured in ChangeLog — MedicationLog was flagged as a prior gap in trackedModels", async () => {
    ...
    await captureRedirect(() => logMedicationAdministered(regimen.id, horse.id, formData({ administered: "true" })))
