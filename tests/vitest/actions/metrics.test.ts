...
import { createWeightEntry, createHorseMetric } from "@/app/horses/[id]/metrics-actions"
import { prisma } from "@/lib/prisma"
import { mockSignedInAs } from "../helpers/auth-mock"
import { createHorse, createVolunteer, getMetricType } from "../helpers/factories"
...
describe("createWeightEntry", () => {
  it("is Admin or Shift Lead — a plain Volunteer is rejected", async () => {
    ...
    await expect(createWeightEntry(horse.id, formData({ weight: "950", context: "ROUTINE" }))).rejects.toThrow("Not authorized")
  ...
  it("logs a routine weigh-in attributed to the Shift Lead who recorded it", async () => {
    ...
    await captureRedirect(() => createWeightEntry(horse.id, formData({ weight: "1024.5", context: "ROUTINE" })))

    const entry = await prisma.weightEntry.findFirstOrThrow({ where: { horseId: horse.id } })
  ...
  it("logs an ad hoc assessment weigh-in", async () => {
    ...
    await captureRedirect(() => createWeightEntry(horse.id, formData({ weight: "800", context: "ASSESSMENT" })))
...
describe("createHorseMetric", () => {
  it("is Admin or Shift Lead — a plain Volunteer is rejected", async () => {
    ...
    await expect(createHorseMetric(horse.id, formData({ metricTypeId: metricType.id, value: "5" }))).rejects.toThrow("Not authorized")
    expect(await prisma.horseMetric.count()).toBe(0)
  })
  ...
  it("logs a half-point Henneke BCS score", async () => {
    ...
    await captureRedirect(() => createHorseMetric(horse.id, formData({ metricTypeId: metricType.id, value: "4.5", notes: "ribs easily felt" })))
  ...
  it("logs a height metric stored as hands.inches notation, not converted", async () => {
    ...
    await captureRedirect(() => createHorseMetric(horse.id, formData({ metricTypeId: metricType.id, value: "15.2" })))
