...
import { createCareEntry, createHealthIssue, resolveHealthIssue } from "@/app/horses/[id]/care-actions"
import { prisma } from "@/lib/prisma"
import { mockSignedInAs } from "../helpers/auth-mock"
import { createHorse, createVolunteer, getCareType } from "../helpers/factories"
...
describe("createCareEntry", () => {
  it("is Admin or Shift Lead — a plain Volunteer is rejected", async () => {
    ...
    await expect(createCareEntry(horse.id, formData({ careTypeId: careType.id }))).rejects.toThrow("Not authorized")
  ...
  it("logs an entry attributed to the Shift Lead who performed it", async () => {
    ...
    const url = await captureRedirect(() => createCareEntry(horse.id, formData({ careTypeId: careType.id, notes: "brushed and picked hooves" })))
  ...
  it("can be tied to an open health issue", async () => {
    ...
    const issue = await prisma.healthIssue.create({ data: { horseId: horse.id, description: "Left hind laceration", startDate: new Date("2026-07-01") } })

    await captureRedirect(() => createCareEntry(horse.id, formData({ careTypeId: careType.id, relatedHealthIssueId: issue.id })))
...
describe("createHealthIssue", () => {
  it("is Admin or Shift Lead — a plain Volunteer is rejected", async () => {
    ...
    await expect(createHealthIssue(horse.id, formData({ description: "Limping on right front" }))).rejects.toThrow("Not authorized")
  ...
  it("opens an active issue starting today", async () => {
    ...
    await captureRedirect(() => createHealthIssue(horse.id, formData({ description: "Limping on right front" })))
...
describe("resolveHealthIssue", () => {
  it("is Admin or Shift Lead — a plain Volunteer is rejected", async () => {
    const horse = await createHorse()
    const issue = await prisma.healthIssue.create({ data: { horseId: horse.id, description: "Cough", startDate: new Date("2026-07-01") } })
    await createVolunteer({ clerkId: "clerk_vol_rh", role: "VOLUNTEER" })
    mockSignedInAs("clerk_vol_rh")

    await expect(resolveHealthIssue(issue.id, horse.id)).rejects.toThrow("Not authorized")
  ...
  it("marks the issue inactive and sets resolvedDate to today", async () => {
    ...
    await captureRedirect(() => resolveHealthIssue(issue.id, horse.id))
