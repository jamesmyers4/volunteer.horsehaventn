...
| SHIFT_LEAD | `CareEntry`, `FeedingOverride`, `CheckIn` (their shift), `MedicationLog`, `HealthIssue` (open/resolve), `WeightEntry`, `HorseMetric` | All shifts/check-ins org-wide |
| VOLUNTEER | Own `CheckIn` | Own data, shift-relevant feeding/pasture info |
| GUEST | Nothing by default | Time-boxed, read-only |

`SHIFT_LEAD` and `VOLUNTEER` never get write access to `FeedingBaseline`, `Horse` core fields, `Volunteer` records/roles, or `PastureAssignment` — those stay Admin-only regardless of how the UI is framed (moving a horse between fields is an Admin-only action per `CONTEXT.md` §10). `MedicationRegimen` (the standing plan, as opposed to `MedicationLog`'s daily entries) is Admin-only too, same split as `FeedingBaseline`/`FeedingOverride`.
...
- `src/app/horses/[id]/metrics-actions.ts` — `createWeightEntry` and `createHorseMetric` (both Admin or Shift Lead — see the Permissions Quick Reference note; this one was an explicit call, not an inferred extension). `[id]/page.tsx`'s "Metrics & Weight" section lists the last 10 `WeightEntry` rows and the last 10 `HorseMetric` rows (joined with `MetricType` for the name/unit) side by side, each with its own log form. `WeightEntry` and `HorseMetric` stay two separate lists in the UI, matching why they're two separate tables per `CONTEXT.md` §13 — weight is the timeline of record other things reference, `HorseMetric` is the generic bucket for Henneke BCS/height/whatever gets added later as a new `MetricType` row.
- `src/app/horses/[id]/pasture-actions.ts` — `assignPasture` (Admin-only). Closes the horse's current active `PastureAssignment` (`endDate: null`, set to today) if one exists, then creates a new one — sequential awaits, no `$transaction`, matching `CONTEXT.md` §10's explicit "fast and simple app-side enforcement, not a DB exclusion constraint" reasoning. `[id]/page.tsx`'s "Field / Pasture" section shows the current field, a short history of past assignments, and the move form.
- `src/app/fields/page.tsx` — the "plain field list" V1 called for in `CONTEXT.md` §1/§10 in place of the Phase 2 interactive drone-photo map. Read-only for any signed-in volunteer: lists all active `Field` rows with turnout/bring-in order and whichever horse(s) currently have an open `PastureAssignment` there (a field can hold more than one horse at once, so this is a list per row, not a single name). Moving a horse still happens from that horse's own detail page, not from here.
- `src/app/dashboard/page.tsx` — the cross-entity daily dashboard: one row per `ACTIVE` horse showing today's feeding (baseline + any override), today's medication status (given/missed/pending per regimen), open health issues plus the most recent care entry, latest weight, and current pasture, all in a single table. Read-only for any signed-in volunteer, same access pattern as `horses/page.tsx` and `fields/page.tsx` — no new Server Actions, it only reads. Queries the last care entry and last weight entry per horse with `orderBy: [{ horseId: "asc" }, { date: "desc" }], distinct: ["horseId"]` (Postgres `DISTINCT ON` under the hood) rather than one query per horse, to keep it to a fixed number of round trips regardless of herd size.
